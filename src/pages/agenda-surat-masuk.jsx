...
import { getSuratMasuk } from "@/services/suratMasukService"; // Pastikan ada service ini
import jsPDF from "jspdf";
import autoTable from "jspdf-autotable";

function AgendaSuratMasuk() {
  ...
  const filteredByDate = filteredData.filter((row) => {
    const tgl = row.tanggal_diterima ? new Date(row.tanggal_diterima) : null;
  ...
  const handlePrintPDF = () => {
    const doc = new jsPDF();
    doc.text("Agenda Surat Masuk", 14, 10);
    autoTable(doc, {
      head: [
        [
          "Nomor Surat",
    ...
    doc.save("agenda-surat-masuk.pdf");
  };
  ...
  return (
    <div className="flex h-screen">
      ...
      <main className="flex-1 bg-gray-100 p-8 flex flex-col items-center justify-center">
        <section>
          <AppTitlePage title="Agenda Surat Masuk" Icon={MailPlus} />
        </section>
